    ...
    if beam_preds is not None:
        for k in top_k:
            ...
            for beams, target in zip(beam_preds, labels):
                if target.strip() in [pred.strip() for pred in beams[:k]]:
                    correct += 1
...
def evaluate_completion(model, tokenizer, eval_loader, num_tokens_to_predict=1, num_beams=5):
    all_preds = []
    all_beam_preds = []  # Store all beam predictions for pass@k
    all_targets = []
    ...
    for batch in tqdm(eval_loader, desc="Evaluating"):
        ...
        for i in range(input_ids.shape[0]):
            ...
            # Generate completion with beam search
            with torch.no_grad():
                ...
                # Process all beam predictions
                beam_preds = []
                for beam_idx in range(num_beams):
                    beam_output = outputs[beam_idx]
                    pred = beam_output[cut_point:cut_point + num_tokens_to_predict]
                    pred_text = tokenizer.decode(pred, skip_special_tokens=True)
                    beam_preds.append(pred_text)

                # Store all beam predictions
                all_beam_preds.append(beam_preds)
                # Store best prediction (first beam) for other metrics
